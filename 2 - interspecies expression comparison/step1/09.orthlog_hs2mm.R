...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "mmusculus_metatable.csv")
...
#saveRDS(hs2mm.orth, "./09.mmusculus/09.hs2mm.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./09.mmusculus/09.hs2mm.orth.rds")
...
#Original gene number: 17140
#Filtered gene number: 15726
...
#[1] 15726     4
write.csv(hs2mm.orth.flt, file ="./09.mmusculus/hs2mmusculus.orth.inf.csv",row.names = FALSE)
head(hs2mm.orth.flt,5)
head(hs2mm.data)
# 将表达数据与过滤后的同源基因数据合并
...
write.csv(hs2mm.hsHigh, file = "./09.mmusculus/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./09.mmusculus/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./09.mmusculus/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./09.mmusculus/09.mmusculus.MA.svg")
...
svg("./09.mmusculus/09.mmusculus.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
