...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "ppaniscus_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
hs2mm.orth <- ortholog_match("human", "bonobo")
saveRDS(hs2mm.orth, "./11.ppaniscus/11.hs2ppaniscus.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./11.ppaniscus/11.hs2ppaniscus.orth.rds")
...
write.csv(hs2mm.hsHigh, file = "./11.ppaniscus/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./11.ppaniscus/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./11.ppaniscus/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./11.ppaniscus/11.ppaniscus.MA.svg")
...
svg("./11.ppaniscus/11.ppaniscus.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
