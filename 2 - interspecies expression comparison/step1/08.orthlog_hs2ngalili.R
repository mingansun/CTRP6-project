...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "ngalili_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
hs2mm.orth <- ortholog_match("human", "ngalili")
saveRDS(hs2mm.orth, "./08.ngalili/08.hs2ngalili.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./08.ngalili/08.hs2ngalili.orth.rds")
...
write.csv(hs2mm.hsHigh, file = "./08.ngalili/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./08.ngalili/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./08.ngalili/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./08.ngalili/08.ngalili.MA.svg")
...
svg("./08.ngalili/08.ngalili.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
