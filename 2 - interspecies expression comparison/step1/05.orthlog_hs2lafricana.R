...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "lafricana_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
hs2mm.orth <- ortholog_match("human", "elephant")
saveRDS(hs2mm.orth, "./05.lafricana/05.hs2lafricana.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./05.lafricana/05.hs2lafricana.orth.rds")
...
#Original gene number: 16083
#Filtered gene number: 14731
# 检查过滤前后的基因数量
print(dim(hs2mm.orth[[1]]))  # 过滤前的数量
#[1] 17140     4
print(dim(hs2mm.orth.flt[[1]]))  # 过滤后的数量
#[1] 15726     4
write.csv(hs2mm.orth.flt, file ="./05.lafricana/hs2lafricana.orth.inf.csv",row.names = FALSE)
#head(hs2mm.orth.flt)
...
write.csv(hs2mm.hsHigh, file = "./05.lafricana/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./05.lafricana/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./05.lafricana/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./05.lafricana/05.lafricana.MA.svg")
...
svg("./05.lafricana/05.lafricana.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
