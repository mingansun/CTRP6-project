...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "dnovemcinctus_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
hs2mm.orth <- ortholog_match("human", "armadillo")
saveRDS(hs2mm.orth, "./03.dnovemcinctus/03.hs2dnovemcinctus.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./03.dnovemcinctus/03.hs2dnovemcinctus.orth.rds")
...
#[1] 14211     4
write.csv(hs2mm.orth.flt, file ="./03.dnovemcinctus/hs2dnovemcinctus.orth.inf.csv",row.names = FALSE)
# 将表达数据与过滤后的同源基因数据合并
...
write.csv(hs2mm.hsHigh, file = "./03.dnovemcinctus/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./03.dnovemcinctus/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./03.dnovemcinctus/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./03.dnovemcinctus/03.dnovemcinctus.MA.svg")
...
svg("./03.dnovemcinctus/03.dnovemcinctus.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
