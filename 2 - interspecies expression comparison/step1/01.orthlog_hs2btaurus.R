...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "btaurus_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
#hs2mm.orth <- ortholog_match("human", "cow")
#saveRDS(hs2mm.orth, "./01.btaurus/01.hs2btaurus.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./01.btaurus/01.hs2btaurus.orth.rds")
...
#Original gene number: 16699
#Filtered gene number: 15304
...
write.csv(hs2mm.orth.flt, file ="./01.btaurus/hs2btaurus.orth.inf.csv",row.names = FALSE)
head(hs2mm.data)
...
write.csv(hs2mm.hsHigh, file = "./01.btaurus/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./01.btaurus/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./01.btaurus/hs2mm.all_gene.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./01.btaurus/01.btaurus.MA.svg")
...
svg("./01.btaurus/01.btaurus.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
