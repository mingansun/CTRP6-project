...
# 定义元数据文件的路径
hs2mm_meta_file <- file.path(data_dir, "oaries_metatable.csv")

# 生成 ExprX 数据集
hs2mm.data <- make_ExprX_dataset(hs2mm_meta_file)

# 匹配直系同源基因
hs2mm.orth <- ortholog_match("human", "oaries")
saveRDS(hs2mm.orth, "./10.oaries/10.hs2oaries.orth.rds")

# 读取保存的直系同源基因数据
hs2mm.orth <- readRDS(file = "./10.oaries/10.hs2oaries.orth.rds")
...
write.csv(hs2mm.hsHigh, file = "./10.oaries/hs2mm_hsHigh.csv", row.names = FALSE)
write.csv(hs2mm.mmHigh, file = "./10.oaries/hs2mm_mmHigh.csv", row.names = FALSE)
write.csv(hs2mm.deg, file ="./10.oaries/hs2mm.all_gene.inf.csv",row.names = FALSE)

#list_species()
#write.csv(list_species(), file ="D:/Du/TPM/ducui/all_spe.inf.csv",row.names = FALSE)

# Generate MA-plot
svg("./10.oaries/10.oaries.MA.svg")
...
svg("./10.oaries/10.oaries.Volcano.svg")
ortholog_expression_plot(
  hs2mm.deg, "volcano",
